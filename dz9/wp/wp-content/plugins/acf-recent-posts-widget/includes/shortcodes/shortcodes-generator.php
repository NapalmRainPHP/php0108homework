...
function acf_rpw_define_shortcode() {
	...
	$test
			...
			->add_field( 'checkbox', 'is', __( 'Ignore sticky posts', 'acf_rpw' ), '', array( 'is' => __( 'Ignore', 'acf_rpw' ) ) )
			...
			->add_field( 'checkbox', 'dd', __( 'Display Date', 'acf_rpw' ), '', array( 'dd' => __( 'Display Date', 'acf_rpw' ) ) )
			->add_field( 'checkbox', 'dlm', __( 'Display Modified Date', 'acf_rpw' ), __( 'Checked - displays the last modified date of the post. Settings below apply.', 'acf_rpw' ), array( 'dlm' => __( 'Display Modified Date', 'acf_rpw' ) ) )
			...
			->add_field( 'checkbox', 'dr', __( 'Date Relative', 'acf_rpw' ), __( 'Checked - ignores the date format. Displays date in relateive format ex: 2 minutes ago.', 'acf_rpw' ), array( 'dr' => __( 'Date Relative', 'acf_rpw' ) ) )
			...
			->add_field( 'checkbox', 'hp', __( 'Show password protected posts only?', 'acf_rpw' ), '', array( 'hp' => __( 'Has Password', 'acf_rpw' ) ) )
			->add_field( 'checkbox', 'ep', __( 'Exclude password protected posts?', 'acf_rpw' ), __( 'Has lowest priority over the other password fields!', 'acf_rpw' ), array( 'ep' => __( 'No Password', 'acf_rpw' ) ) )
...
// print the post formats checkboxes
	...
	if ( !is_wp_error( $tags_get ) ) {
		...
		$test->add_field( 'checkbox', 'lttag', __( 'Limit to Tag', 'acf_rpw' ), '', $tags );
	}
...
// thumbnail related settings
	if ( current_theme_supports( 'post-thumbnails' ) ) {
		$test->add_field( 'checkbox', 'dth', __( 'Display Thumbnail', 'acf_rpw' ), __( 'Needs to be set as post featured image', 'acf_rpw' ), array( 'display' => __( 'Display', 'acf_rpw' ) ) )
	...
	$test->add_field( 'text', 'dfth', __( 'Default Thumbnail', 'acf_rpw' ), __( 'Specify full, valid image URL here. Ex: http://placehold.it/50x50/f0f0f0/ccc. All of the above apply to thumbnails but not to ACF image field type. Use CSS "acf-img" class to reference these.', 'acf_rpw' ) )
			...
			->add_field( 'checkbox', 'rm', __( 'Display Readmore', 'acf_rpw' ), '', array( 'display' => __( 'Readmore', 'acf_rpw' ) ) )
	...
	$test->add_field( 'textarea', 'before', __( 'HTML or text before each post.', 'acf_rpw' ), __( 'You can use any HTML and meta / ACF keys here. [acf field_key] will render the corresponding ACF field\'s value. Meta can be obtained via [meta field_key].', 'acf_rpw' ) );
	$test->add_field( 'textarea', 'after', __( 'HTML or text after each post.', 'acf_rpw' ), __( 'You can use any HTML and meta / ACF keys here. [acf field_key] will render the corresponding ACF field\'s value. Meta can be obtained via [meta field_key].', 'acf_rpw' ) );
	$test->add_field( 'textarea', 'before_posts', __( 'HTML or text before the whole loop.', 'acf_rpw' ), __( 'You can use any HTML here, the markup appears after the widget container opening and after the title.', 'acf_rpw' ) );
	$test->add_field( 'textarea', 'after_posts', __( 'HTML or text after the whole loop.', 'acf_rpw' ), __( 'You can use any HTML here, the markup appears before the widget container closing.', 'acf_rpw' ) );
	$test->add_field( 'checkbox', 'default_styles', __( 'Use Default Styles', 'acf_rpw' ), '', array( 'default_styles' => __( 'Default', 'acf_rpw' ) ) );
	$test->add_field( 'textarea', 'custom_css', __( 'Custom CSS', 'acf_rpw' ), __( 'Disabling default CSS will let you type in any CSS here.', 'acf_rpw' ) );
